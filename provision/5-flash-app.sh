...
SSID="" PASS="" SUP="" PORT="" KEYS="mainToken,backupToken" FEATURES="udp-transport,efuse-hmac-identity" OFF="$APP_OFFSET_DEFAULT" ENCRYPT=0
while [ $# -gt 0 ]; do case "$1" in
  ...
  # Use on a board that already has flash encryption enabled (dev mode, "N plaintext
  # flashes left") but is NOT yet release-sealed. The ROM encrypts the write, so the
  # decrypt-on-boot yields the right image. NEVER flash such a board plaintext — the
  # bootloader then decrypts garbage ("invalid magic byte" boot loop). Adds
  # --encrypt and --no-stub (Secure Download Mode has no flasher stub). No effect on a
  # non-encrypted board; won't work once DIS_DOWNLOAD_MANUAL_ENCRYPT is burned (stage 6).
...
if [ "$ENCRYPT" = 1 ]; then
  esptool --chip esp32s3 --port "$PORT" --no-stub --after hard_reset write_flash --encrypt "$OFF" "$WORK/app-signed.bin"
else
